...
int main()
{
	...
	while(1)
	{
		...
		if (productor!=NULL)
		{
			...
			{
				memset(variableProducto,0x00,sizeof(variableProducto));
				memset(cadena,0x00,sizeof(cadena));
				printf("INGRESE NOMBRE PRODUCTO\n")
				scanf("%s",&variableProducto);
				sprintf(cadena, "PRODUCTO-%s-%04d-%04d\n",variableProducto, nro_partida, nro_producto);
				printf("%s", cadena);
